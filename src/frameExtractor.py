...
def main():
    ...
    rbg_camera_extrinsics = calib_device.get_transform_cpf_sensor(stream_label_rgb).to_matrix()
    ...
    for time in range(t_first, t_last, 1000_000_000):
        ...
        img_et = provider.get_image_data_by_time_ns(stream_id_et, time, TimeDomain.DEVICE_TIME, TimeQueryOptions.CLOSEST)[0].to_numpy_array()
        ...
        if len(imgs)>0 and confidence(img, imgs[-1])<0.7:
            ...
            if blurryness(img) < blurryness(imgs[-1]):
                imgs[-1] = img 
                print(f"INFO: Frame substituted to the last in the list for better sharpness.")
    ...
    for index, img in enumerate(imgs):
        ...
        with open(os.path.join(output_folder,f"img{index}.json"), "w") as f:
            json.dump(
                {'gaze_center_in_cpf':gaze_center_in_cpf.tolist(),
                 'gaze_center_in_rgb_pixels': gaze_center_in_pixels.tolist(),
                 'gaze_center_in_rgb_frame': (np.linalg.inv(rbg_camera_extrinsics)@np.append(gaze_center_in_cpf, [1]))[:3].tolist(), 
                 'rbg_camera_extrinsics': rbg_camera_extrinsics.tolist(),
