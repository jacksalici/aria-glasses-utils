...
class BetterEyeGaze:
    def __init__(
        ...
        self.live, self.correct_distortion, self.rotate_image = (
            live,
            correct_distorsion,
            rotate_image,
        )
        ...
        if not live:
            ...
            self.stream_labels = {
                k: self.provider.get_label_from_stream_id(v)
                for k, v in self.stream_ids.items()
            }
            self.t_first = self.provider.get_first_time_ns(
                self.stream_ids["rgb"], TimeDomain.DEVICE_TIME
            )
            self.t_last = self.provider.get_last_time_ns(
                self.stream_ids["rgb"], TimeDomain.DEVICE_TIME
            )
        ...
        self.calib_rgb_camera_original = self.calib_device.get_camera_calib(
            self.stream_labels["rgb"]
        )
        self.img_w, self.img_h = int(
            self.calib_rgb_camera_original.get_image_size()[0]
        ), int(self.calib_rgb_camera_original.get_image_size()[1])
        ...
        if correct_distorsion:
            ...
            if rotate_image:
                self.calib_rgb_camera_pinhole_cw90 = (
                    calibration.rotate_camera_calib_cw90deg(
                        self.calib_rgb_camera_pinhole
                    )
                )
    ...
    def get_gaze_center_raw(self, yaw, pitch, depth=1.0):
        ...
        transform_cpf_sensor = self.calib_device.get_transform_cpf_sensor(
            self.stream_labels["rgb"]
        )
        gaze_center_in_camera = transform_cpf_sensor.inverse() @ gaze_center_in_cpf

        if self.correct_distortion:
            gaze_center_in_pixels = self.calib_rgb_camera_pinhole.project(
                gaze_center_in_camera
            ).astype(int)
        else:
            gaze_center_in_pixels = self.calib_rgb_camera_original.project(
                gaze_center_in_camera
            ).astype(int)
    ...
    def rotate_pixel_cw90(self, gaze_center_in_pixels, scale=1.0):
        return [
            int(self.img_w * scale) - gaze_center_in_pixels[1],
            gaze_center_in_pixels[0],
        ]

    def get_rgb_image(self, time_ns):

        img = self.provider.get_image_data_by_time_ns(
            self.stream_ids["rgb"],
            time_ns,
        ...
        if self.correct_distortion:
            img = calibration.distort_by_calibration(
                img, self.calib_rgb_camera_pinhole, self.calib_rgb_camera_original
            )
